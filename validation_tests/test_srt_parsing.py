        ...
        for srt_file in srt_files:
            ...
            try:
                ...
                if telemetry["camera_info"]:
                    ...
                    print(
                        f"      📷 Camera: ISO {cam.get('iso', 'N/A')}, {cam.get('shutter', 'N/A')}"
                    )
...
def test_real_srt_parsing():
    ...
    for srt_file in srt_files:
        ...
        try:
            ...
            if telemetry["altitudes"]:
                print(
                    f"      🏔️  Altitude range: {min(telemetry['altitudes']):.1f} - {max(telemetry['altitudes']):.1f}m"
                )

            if telemetry["rel_altitudes"]:
                print(
                    f"      📏 Relative altitude: {min(telemetry['rel_altitudes']):.1f} - {max(telemetry['rel_altitudes']):.1f}m"
                )

            if telemetry["camera_info"]:
                cam = telemetry["camera_info"][0]
                print(
                    f"      📷 Camera: ISO {cam.get('iso', 'N/A')}, shutter {cam.get('shutter', 'N/A')}, f/{cam.get('fnum', 'N/A')}"
                )

            if telemetry["timestamps"]:
                print(
                    f"      ⏱️  Duration: {telemetry['timestamps'][0]} to {telemetry['timestamps'][-1]}"
                )
    ...
    print(
        f"   📊 Results: {success_count}/{len(srt_files)} real SRT files parsed successfully"
    )
    return success_count > 0
    ...
    for format_name, srt_content in formats:
        ...
        try:
            ...
            try:
                ...
                if telemetry["gps_coords"]:
                    print(
                        f"      ✅ GPS points extracted: {len(telemetry['gps_coords'])}"
                    )
                    lat, lon = telemetry["gps_coords"][0]
                    print(f"      📍 First GPS: {lat:.6f}, {lon:.6f}")
                    success_count += 1
