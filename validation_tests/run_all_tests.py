...
def check_prerequisites():
    ...
    if pyproject_file.exists():
        ...
        print(
            "   📝 Make sure you're running from the dji-drone-metadata-embedder directory"
        )
        prerequisites_ok = False
...
def run_all_validation_tests():
    ...
    if passed == total:
        ...
        if not any(
            "Installation" in name for name, success in results.items() if not success
        ):
            print("✅ Core functionality appears to work.")
            print(
                "🚀 You can likely use the tool, but some advanced features may be limited."
            )
