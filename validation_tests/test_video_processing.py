    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if embedder1.output_dir == expected_output:
                ...
                print(
                    f"   ❌ Output directory mismatch: {embedder1.output_dir} != {expected_output}"
                )
                return False
...
def test_video_file_detection():
    ...
    try:
        ...
        video_files = list(drone_footage_dir.glob("*.mp4")) + list(
            drone_footage_dir.glob("*.MP4")
        )
        srt_files = list(drone_footage_dir.glob("*.srt")) + list(
            drone_footage_dir.glob("*.SRT")
        )
        ...
        for video_file in video_files[:1]:  # Test first file only
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if telemetry["gps_coords"]:
                    print(
                        f"      ✅ SRT parsing successful ({len(telemetry['gps_coords'])} GPS points)"
                    )
                else:
                ...
                output_file = (
                    output_dir / f"{video_file.stem}_metadata{video_file.suffix}"
                )
...
def simulate_with_samples():
    ...
    try:
        ...
        if telemetry["gps_coords"]:
            print(
                f"      ✅ Sample parsing successful ({len(telemetry['gps_coords'])} GPS points)"
            )
            return True
        else:
...
def test_json_output_format():
    ...
    try:
        ...
        json_data = {
            ...
            "max_relative_altitude": max(test_telemetry["rel_altitudes"])
            if test_telemetry["rel_altitudes"]
            else None,
            "flight_duration": test_telemetry["flight_duration"],
            "num_gps_points": len(test_telemetry["gps_coords"]),
            "camera_settings": test_telemetry["camera_info"][0]
            if test_telemetry["camera_info"]
            else {},
        }
