...
class TestGoldenFixtures:
    ...
    def test_m300_legacy_unit_format(self):
        ...
        try:
            ...
            for i, (exp_lat, exp_lon, _exp_alt) in enumerate(
                sample["expected_coordinates"]
            ):
    ...
    def test_all_golden_samples(self):
        ...
        for sample_name, sample in GOLDEN_SAMPLES.items():
            ...
            try:
                ...
                assert normalization["normalized_count"] > 0, (
                    f"Normalization failed for {sample_name}"
                )
...
class TestValidateDirectoryVideoGlobs:
    ...
    def test_mov_srt_pair_is_counted(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert not any("No SRT file found" in issue for issue in result["issues"])
