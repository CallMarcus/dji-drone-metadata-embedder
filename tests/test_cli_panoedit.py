...
def _stub_nonblocking_logging(monkeypatch):
    ...
    calls: list[bool] = []
    monkeypatch.setattr(
        cli_mod, "make_logging_nonblocking", lambda *a, **kw: calls.append(True)
    )
    return calls
...
def test_panoedit_max_width_is_overridable(monkeypatch, tmp_path):
    ...
    calls = {}
    monkeypatch.setattr(cli_mod, "run_editor", lambda directory, **kw: calls.update(kw))
    result = CliRunner().invoke(
        cli_mod.main, ["panoedit", str(tmp_path), "--max-width", "0"]
    )
    assert result.exit_code == 0, result.output
...
def test_panoedit_no_backup_reaches_run_editor(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(cli_mod, "run_editor", lambda directory, **kw: calls.update(kw))
    result = CliRunner().invoke(
        cli_mod.main, ["panoedit", str(tmp_path), "--no-backup"]
    )
    assert result.exit_code == 0, result.output
...
def test_panoedit_clean_backups_deletes_and_never_serves(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        cli_mod, "run_editor", lambda *a, **kw: pytest.fail("server must not start")
    )
    result = CliRunner().invoke(
        cli_mod.main, ["panoedit", str(tmp_path), "--clean-backups"]
    )
    assert result.exit_code == 0, result.output
