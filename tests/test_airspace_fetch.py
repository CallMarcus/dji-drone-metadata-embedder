...
def test_luxembourg_fetch_parses_caches_and_announces(tmp_path):
    ...
    lines = []
    data = fetch_zones(
        _track(49.62, 6.2), tmp_path, transport=fake, announce=lines.append
    )
...
def test_second_run_uses_the_cache_offline(tmp_path):
    ...
    data = fetch_zones(
        _track(49.62, 6.2), tmp_path, transport=no_network, announce=lines.append
    )
...
def test_refresh_forces_a_refetch(tmp_path):
    ...
    data = fetch_zones(_track(49.62, 6.2), tmp_path, refresh=True, transport=fake2)
...
def test_an_irish_flight_discovers_the_dated_file_then_fetches_it(tmp_path):
    ...
    lines = []
    data = fetch_zones(
        _track(53.35, -6.26), tmp_path, transport=fake, announce=lines.append
    )
    assert data.gap_reason is None and len(data.zones) == 4
...
def _gb_zip() -> bytes:
    ...
    with zipfile.ZipFile(buf, "w") as zf:
        ...
        zf.writestr(
            ...
            hashlib.sha256(xml).hexdigest() + " *EG_UAS_FR_DS_AREA1_FULL_20260806.xml",
        )
    return buf.getvalue()


GB_PAGE = b'<a href="/x/UAS_AREA_1/EG_UAS_FR_DS_AREA1_FULL_20200101_XML.zip"></a>'
...
def test_a_uk_flight_discovers_the_cycle_zip_and_caches_the_xml(tmp_path):
    ...
    lines = []
    data = fetch_zones(
        _track(51.50, -0.12), tmp_path, transport=fake, announce=lines.append
    )
    ...
    assert any("Fetching" in ln and "nats-uk.ead-it.com" in ln for ln in lines)
    ...
    meta = json.loads((tmp_path / "aixm-GB.xml.meta.json").read_text(encoding="utf-8"))
    assert meta["effective"] == "2020-01-01"


def test_a_cached_uk_body_skips_discovery_and_the_network(tmp_path):
    fetch_zones(
        _track(51.50, -0.12), tmp_path, transport=FakeTransport([GB_PAGE, _gb_zip()])
    )
...
def test_a_danish_flight_discovers_the_geojson_and_caches_it(tmp_path):
    fake = FakeTransport([DK_PAGE, (FIXTURES / "dronezoner-dk.json").read_bytes()])
    lines = []
    data = fetch_zones(
        _track(55.68, 12.57), tmp_path, transport=fake, announce=lines.append
    )
    assert data.gap_reason is None and len(data.zones) == 7
    assert fake.urls[0].startswith("https://www.en.droneregler.dk/")
    assert fake.urls[1].endswith("items/f049a65ae2f34bd1b747895d555dac71/data")
    assert data.source is not None
    assert "kildeangivelse" in data.source.license
    assert (tmp_path / "dronezoner-DK.json").exists()
    assert any("Fetching" in ln and "www.en.droneregler.dk" in ln for ln in lines)
...
def test_a_swedish_flight_fetches_the_lfv_file_directly(tmp_path):
    ...
    lines = []
    data = fetch_zones(
        _track(59.33, 18.07), tmp_path, transport=fake, announce=lines.append
    )
    assert data.gap_reason is None and len(data.zones) == 3
    assert fake.urls == ["https://dronechart.lfv.se/data/uas_zones_ED318.json"]
...
def _si_zip() -> bytes:
    ...
    with zipfile.ZipFile(outer, "w") as zf:
        info = zipfile.ZipInfo(
            "UAS Geo zones - May_2026.kmz", date_time=(2026, 5, 25, 13, 39, 0)
        )
...
def test_a_slovenian_flight_discovers_the_zip_then_fetches_it(tmp_path):
    ...
    lines = []
    data = fetch_zones(
        _track(46.05, 14.51), tmp_path, transport=fake, announce=lines.append
    )
...
def test_a_cached_slovenian_body_skips_discovery_and_the_network(tmp_path):
    fetch_zones(
        _track(46.05, 14.51), tmp_path, transport=FakeTransport([SI_PAGE, _si_zip()])
    )
...
def test_an_estonian_flight_fetches_the_eans_file_directly(tmp_path):
    ...
    lines = []
    data = fetch_zones(
        _track(59.44, 24.75), tmp_path, transport=fake, announce=lines.append
    )
    assert data.gap_reason is None and len(data.zones) == 3
...
def _be_track(timed=True):
    ...
    return Track(
        ...
        points=[
            TrackPoint(
                lat=50.85, lon=4.35, alt=100, timestamp="c", utc=start, rel_alt=30
            ),
            TrackPoint(
                lat=50.86, lon=4.36, alt=100, timestamp="c", utc=end, rel_alt=30
            ),
        ],
    )
