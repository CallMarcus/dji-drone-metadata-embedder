...
_MARKER_JS = (
    "() => map.getSource('gaze-marker').serialize().data.features"
...
def test_marker_layer_mounts_with_the_cursor_dot(serve_map, page):
    serve_map(flights_to_3d_html([_flight("DJI_0001", 10.0, 20.0, [50.0] * 6)], "trip"))
    _ready(page)
    assert page.evaluate("() => !!map.getLayer('gaze-cursor-dot')")
    assert (
        page.evaluate("() => map.getLayer('gaze-marker-body').type") == "fill-extrusion"
    )
...
def test_marker_sits_at_true_altitude_over_the_terrain(serve_map, page):
    ...
    page.wait_for_function(
        ...
        " && Math.abs(groundRef(flights[0]).elev) < 1",
        timeout=20000,
    )
    ...
    serve_map(
        flights_to_3d_html(
            [
                ...
                _flight("DJI_0002", 10.05, 20.05, [50.0] * 6),
            ],
            "trip",
        )
    )
...
def test_no_marker_without_altitude(serve_map, page):
    ...
    serve_map(flights_to_3d_html([_flight("DJI_0001", 10.0, 20.0, [None] * 6)], "trip"))
    _ready(page)
...
def test_marker_follows_the_dot_under_fuzz(serve_map, page):
    ...
    serve_map(
        flights_to_3d_html(
            [_flight("DJI_0001", 10.0, 20.0, [50.0] * 6)], "trip", redact="fuzz"
        )
    )
    _ready(page)
