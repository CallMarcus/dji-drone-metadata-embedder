...
def _flight(
    name: str, points: int = 6, *, media=None, segments=None, seg_of=None
) -> Track:
...
def _serve_with_video(serve_map, recorded_webm, track, **kw):
    return serve_map(
        flights_to_3d_html([track], "trip", **kw),
...
def test_video_and_slider_appear_in_the_cockpit(serve_map, page, recorded_webm):
    _serve_with_video(serve_map, recorded_webm, _flight("DJI_0001", media=[VIDEO_NAME]))
    _ready(page)
...
def test_crossing_a_segment_boundary_swaps_the_source(serve_map, page, recorded_webm):
    ...
    serve_map(
        flights_to_3d_html([track], "trip"),
        extra_files={
            "a/" + VIDEO_NAME: recorded_webm,
    ...
    page.evaluate("() => ghostEnter(0, 0)")
    _wait_video_ready(page)
    assert page.evaluate("() => document.getElementById('ghost-video').src").endswith(
        "a/" + VIDEO_NAME
    )
...
def test_a_segment_without_video_disables_the_blend(serve_map, page, recorded_webm):
    track = _flight(
        "DJI_0001",
        points=6,
        media=[VIDEO_NAME, None],
        segments=["clip", "missing"],
        seg_of=[0, 0, 0, 1, 1, 1],
    )
    ...
    page.evaluate("() => { ghost.idx = 4; renderCrossfade(); }")
    assert (
        page.evaluate("() => document.getElementById('ghost-blend').disabled") is True
    )
    assert (
        page.evaluate(
            "() => getComputedStyle(document.getElementById('ghost-video')).display"
        )
        == "none"
    )
...
def test_a_sample_without_a_timecode_hides_the_video_and_says_why(
    ...
    page.wait_for_function(
        ...
        ".textContent.includes('timecode')",
        timeout=5000,
    )
    assert (
        page.evaluate("() => document.getElementById('ghost-video').style.display")
        == "none"
    )
    assert (
        page.evaluate("() => document.getElementById('ghost-blend').disabled") is True
...
def test_a_timecode_past_the_video_end_says_so(serve_map, page, recorded_webm):
    ...
    page.wait_for_function(
        "() => document.getElementById('ghost-badges')"
        ".textContent.includes('video ends')",
        timeout=10000,
    )
    assert (
        page.evaluate("() => document.getElementById('ghost-video').style.display")
...
def test_plays_in_sync_at_normal_speed(serve_map, page, recorded_webm):
    ...
    assert (
        page.evaluate("() => document.getElementById('ghost-video').playbackRate") == 1
    )
...
def test_fast_playback_seeks_instead_of_playing(serve_map, page, recorded_webm):
    ...
    assert page.evaluate("() => document.getElementById('ghost-video').paused") is True
...
def test_v_key_under_fuzz_says_why_instead_of_mutating(serve_map, page, recorded_webm):
    ...
    _serve_with_video(
        serve_map, recorded_webm, _flight("DJI_0001", media=[VIDEO_NAME]), redact="fuzz"
    )
    _ready(page)
    page.evaluate("() => ghostEnter(0, 0)")
    page.keyboard.press("v")
...
def test_v_key_on_a_disabled_blend_leaves_state_alone(serve_map, page):
    ...
    serve_map(flights_to_3d_html([_flight("DJI_0001", media=["gone.webm"])], "trip"))
    _ready(page)
    page.evaluate("() => ghostEnter(0, 0)")
    page.wait_for_function(
        "() => document.getElementById('ghost-blend').disabled === true", timeout=10000
    )
    page.keyboard.press("v")
    assert page.evaluate("() => cross.blend") == 0
...
def test_missing_focal_length_marks_the_alignment_approximate(
    serve_map, page, recorded_webm
):
