...
def _wait_layers(page):
    page.wait_for_function(
        ...
        " && !!map.getLayer('airspace-volume')",
        timeout=20000,
    )


def test_agl_ceiling_is_an_exact_volume(serve_map, page):
    html = flights_to_3d_html([_flight()], "t", airspace_json=_overlay([_zone()]))
...
def test_amsl_ceiling_settles_to_terrain_relative_height(serve_map, page):
    ...
    html = flights_to_3d_html([_flight()], "t", airspace_json=_overlay([zone]))
    serve_map(html, terrain_stub=100.0)
    _wait_layers(page)
...
def test_zone_click_opens_published_facts_popup(serve_map, page):
    ...
    page.wait_for_function(
        "() => map.queryRenderedFeatures({layers: ['airspace-footprint']}).length > 0",
        timeout=20000,
    )
    pos = page.evaluate(
        "() => { const p = map.project([20.005, 10.005]); return { x: p.x, y: p.y }; }"
    )
    page.mouse.click(pos["x"], pos["y"])
    expect(page.locator(".flight-popup")).to_contain_text("Test zone", timeout=10000)
...
def test_entered_zone_lands_in_the_entered_layer(serve_map, page):
    entered = [
        {
            "flight": "DJI_0001",
            "entry_utc": "2026-06-15 12:00:10 UTC",
    ...
    assert page.evaluate("() => map.getFilter('airspace-volume-entered')") == [
        "get",
        "entered",
    ]
    assert page.evaluate("() => map.getFilter('airspace-volume')") == [
        "!",
        ["get", "entered"],
    ]
...
def test_long_airspace_notes_wrap_inside_a_capped_panel(serve_map, page):
    ...
    long_note = (
        "Airspace note: Published by LFV with Transportstyrelsen "
    ...
    html = flights_to_3d_html(
        [_flight()],
        "t",
        airspace_json=_overlay(
            [_zone()],
            notes=[
                (
                    "Airspace: Sweden UAS geographical zones (ED-318, LFV), "
                    "fetched 2026-08-22T12:44:39Z"
                ),
                long_note,
            ],
        ),
...
def test_airspace_toggle_hides_all_layers(serve_map, page):
    ...
    for layer in ("airspace-volume", "airspace-volume-entered", "airspace-footprint"):
        assert (
