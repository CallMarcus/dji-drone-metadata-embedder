...
class DiagnosticTool:
    ...
    def check_project_structure(self):
        ...
        if (self.project_root / "dji_metadata_embedder").exists() and (
            self.project_root / "src" / "dji_metadata_embedder"
        ).exists():
            self.log_issue(
                "Duplicate package locations found: /dji_metadata_embedder and /src/dji_metadata_embedder"
            )

        # Check for correct package location
        if not (self.project_root / "src" / "dji_metadata_embedder").exists():
            self.log_issue(
                "Package not found in expected location: /src/dji_metadata_embedder"
            )
    ...
    def check_imports(self):
        ...
        try:
            ...
            self.log_info(
                f"Package imported successfully, version: {getattr(dji_metadata_embedder, '__version__', 'unknown')}"
            )
    ...
    def check_entry_points(self):
        ...
        result = subprocess.run(
            ["dji-embed", "--version"], capture_output=True, text=True
        )
        if result.returncode == 0:
            ...
            result = subprocess.run(
                [sys.executable, "-m", "dji_metadata_embedder", "--version"],
                capture_output=True,
                text=True,
            )
            if result.returncode == 0:
    ...
    def check_git_status(self):
        ...
        if result.returncode == 0:
            if result.stdout:
                self.log_warning(
                    f"Uncommitted changes: {len(result.stdout.splitlines())} files"
                )
            else:
