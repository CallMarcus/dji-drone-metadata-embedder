...
def _zone_row(f: ZoneFinding) -> str:
    ...
    if not f.entered:
        ...
        if stated is None:
            ...
            if f.max_surface_m is not None:
                compare = "est. max height above surface during dwell: " + _both_units(
                    f.max_surface_m, stated.unit
                )
            else:
                compare = (
            ...
            if f.max_amsl_m is not None:
                compare = "max altitude (AMSL) during dwell: " + _both_units(
                    f.max_amsl_m, stated.unit
                )
            else:
    ...
    if z.notes:
        ...
        name_cell += (
            f"<br><small>published, not evaluated: {_esc('; '.join(z.notes))}</small>"
        )
    return (
...
def _svg(rec: FlightRecordData) -> str:
    ...
    zone_polys = "".join(
        "<polygon points='"
        + " ".join(xy(la, lo) for lo, la in ring)
...
def _cover_row(rec: FlightRecordData) -> str:
    ...
    if rec.airspace.gap_reason:
        ...
        airspace_summary = f"entered {n_entered} zone{'s' if n_entered != 1 else ''}"
    else:
...
def _not_applicable_table(rec: FlightRecordData) -> str:
    ...
    return (
        ...
        "<tr><th>Zone</th><th>Identifier</th><th>Reason</th></tr>" + rows + "</table>"
    )
...
def _source_dl(source, terrain_source: str | None, version: str) -> str:
    ...
    parts = "".join(f"<dt>{_esc(k)}</dt><dd>{_esc(v)}</dd>" for k, v in items)
...
def _flight_section(rec: FlightRecordData, version: str) -> str:
    ...
    gap_html = ""
    if rec.airspace.gap_reason:
        gap_html = f"<p class='gap-note'>{_esc(rec.airspace.gap_reason)}</p>"
    ...
    if rec.airspace.source is not None or rec.terrain_source is not None:
        source_html = "<h3>Data &amp; caveats</h3>" + _source_dl(
            rec.airspace.source, rec.terrain_source, version
        )
...
def record_to_html(records: list[FlightRecordData], title: str, version: str) -> str:
    ...
    cover = (
        ...
        "<th>Airspace</th></tr>" + cover_rows + "</table>"
    )
    ...
    return stamp(
        ...
        f"<h1>{_esc(title)}</h1>" + cover + sections + "</body></html>"
    )


def write_flight_record(records: list[FlightRecordData], out: Path, title: str) -> Path:
    from dji_metadata_embedder import __version__

    out.write_text(record_to_html(records, title, __version__), encoding="utf-8")
    return out
